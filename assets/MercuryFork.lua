...
function Library:set_status_color(color)
...
function Library:create(options)
	...
	if readfile and writefile and isfile then
        ...
        if toggleKey then
            local split = tostring(toggleKey):split(".")

            if split[1] == "Enum" then
                Library.ToggleKey = Enum[split[2]][split[3]]
            end
        end
	...
	local homeButtonIcon = homeButton:object("ImageLabel", {
		...
		Image = options.Icon or "http://www.roblox.com/asset/?id=8569322835",
		Theme = {ImageColor3 = "StrongText"}
	})
    ...
    self:change_theme(Library.CurrentTheme)
    ...
    creditsTab:credit{Name = "3xjn", Description = "Forker", Discord = "asher.#2200"}
...
function Library:notification(options)
	...
	local icon = noti:object("ImageLabel", {
		...
		Image = options.Icon or "rbxassetid://8628681683",
		Theme = {ImageColor3 = "Tertiary"}
	})
		...
		UserInputService.InputBegan:Connect(function(key)
			if listening then
				if key.UserInputType == Enum.UserInputType.Keyboard then
                    ...
                    if options.Name == "Toggle Key" then
                        updateSettings("ToggleKey", tostring(key.KeyCode))
                    end
				end
			else
